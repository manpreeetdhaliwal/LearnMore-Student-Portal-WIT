...
function assignmentTitle() {
    ...
    document.getElementById('quizTitle').innerHTML = urlParams.get('courseTitle');
}

function assignmentPoint() {
    var urlParams = new URLSearchParams(window.location.search);

    document.getElementById('quizPoint').innerHTML = urlParams.get('coursePoint');
}

function assignmentDueDate() {
    var urlParams = new URLSearchParams(window.location.search);

    document.getElementById('quizDuedate').innerHTML = urlParams.get('courseDuedate');
}
...
quizsub.addEventListener('click', e => {
    e.preventDefault();
    alert("You have succesfully submitted your quiz")
